...
class ModelEMA:
    ...
    def __init__(
        ...
        self.ema_params = [
            copy.deepcopy(p).detach().requires_grad_(False)
...
def get_pretrained_flux2vae() -> torch.nn.Module:
    ...
    model.encode = lambda x: (
        orig_encode(x.repeat(1, 3, 1, 1))
        .latent_dist
        .sample()
    )
...
def _get_latent_normalization(
    ...
    if not torch.isfinite(mean).all():
        raise ValueError(
            "Latent normalization mean contains NaN/Inf."
        )

    if not torch.isfinite(std).all():
        raise ValueError(
            "Latent normalization std contains NaN/Inf."
        )

    if torch.any(std <= 0):
        raise ValueError(
            "Latent normalization std contains non-positive values."
        )
...
def _decode_with_vae(
    ...
    # Raw image flow: no VAE decoding.
    if vae is None:
        return x

    if latent_mean is None or latent_std is None:
        raise ValueError(
            "VAE decoding for latent flow requires "
            "latent_mean and latent_std."
        )
    ...
    latent_mean = latent_mean.to(
        device=vae_device,
        dtype=torch.float32,
    )

    latent_std = latent_std.to(
        device=vae_device,
        dtype=torch.float32,
    )
    ...
    if x.ndim != 4:
        raise ValueError(
            f"Expected latent BCHW tensor, got {tuple(x.shape)}."
        )

    if latent_mean.shape[1] != x.shape[1]:
        raise ValueError(
            f"Latent mean channel mismatch: "
...
def _decode_pair_with_vae(
    ...
    if x1.shape[1] != latent_mean.shape[1]:
        raise ValueError(
            "Latent channel mismatch between x and normalization."
        )
...
def _save_image_grid(
    ...
    if x.ndim != 4:
        raise ValueError(
            f"Expected BCHW tensor for image grid, got {tuple(x.shape)}."
        )

    if x.shape[1] not in {1, 3, 4}:
        raise ValueError(
            f"Cannot display tensor with {x.shape[1]} channels. "
...
def _sample_random_interventions(
    ...
    if len(pa) == 0:
        raise ValueError(
            "No parent keys found for counterfactual plotting."
        )
...
def _show_image(
    ...
    if img.ndim != 3:
        raise ValueError(
            f"Expected 2D or 3D image tensor, got shape {tuple(img.shape)}"
        )
    ...
    if img.shape[0] == 1:
        ...
        raise ValueError(
            f"Unsupported channel count: {img.shape[0]}"
        )
...
def _make_effect_map(
    ...
    if diff.ndim != 4:
        raise ValueError(
            f"Expected BCHW tensor, got shape {tuple(diff.shape)}"
        )
...
def _plot_counterfactual_triplets(
    ...
    for idx, ax in enumerate(axes.flatten()):
        ...
        # ----------------------------------------------------
            ...
            ax.set_xlabel(
                f"do({k}={do_pa_named[k][sample_idx]})",
                labelpad=8,
            )
...
def plot_counterfactuals(
    ...
    **kwargs,
    ...
    do_pa, do_keys = _sample_random_interventions(
        pa,
    ...
    #
    # z_raw = z_norm * train_std + train_mean
    #
    # then FLUX decode.
    # ========================================================
    ...
    if vae is not None:
        ...
        print(
            f"VAE inference time elapsed: {time.time() - t0:.2f}s"
        )
...
def save_plots(
    ...
    if latent_mode and vae is None:
        raise ValueError(
            "save_plots(): latent dataset detected but vae=None. "
            "Pass the FLUX.2 VAE to visualize latent-flow outputs."
        )
    ...
    latent_mean, latent_std = _get_latent_normalization(
        dataset,
        device=device,
    )

    if latent_mode:
        print("save_plots: latent flow")
        print("  flow input shape:", tuple(x.shape))
        print(
        ...
        print("save_plots: raw-image flow")
        print("  flow input shape:", tuple(x.shape))
...
def plot_counterfactuals_mf(
    ...
    **kwargs,
    ...
    inv_sample_args = kwargs.pop(
        "inv_sample_args",
        None,
    )

    gen_sample_args = kwargs.pop(
        "gen_sample_args",
        None,
    )

    inv_steps = kwargs.pop(
        "inv_steps",
        150,
    )
...
def save_plots_mf(
    ...
    if latent_mode and vae is None:
        raise ValueError(
            "save_plots_mf(): latent dataset detected but vae=None."
        )
...
def save_reconstructions(
    ...
    if (
        not hasattr(base, "encode")
        or not hasattr(base, "decode")
    ):
        raise AttributeError(
            "save_reconstructions() expects model to have "
            "encode() and decode()."
        )
    ...
    if _is_latent_dataset(dataset):
        raise ValueError(
            "save_reconstructions() is currently for pixel-space "
            "encoder/decoder models, not cached latent-flow datasets."
        )
