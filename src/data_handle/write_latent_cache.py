...
class EmbedImageDataset(Dataset):
    def __init__(
        ...
        self.df = pd.read_csv(self.csv_path, low_memory=False).reset_index(drop=True)
    ...
    def _validate_dataframe(self):
        ...
        if self.df["cache_idx"].isna().any():
            raise ValueError(f"{self.csv_path}: cache_idx contains NaN.")

        cache_idx = self.df["cache_idx"].to_numpy().astype(np.int64)

        if len(np.unique(cache_idx)) != len(cache_idx):
            raise ValueError(f"{self.csv_path}: cache_idx contains duplicates.")

        expected = np.arange(len(self.df), dtype=np.int64)
        actual = np.sort(cache_idx)

        if not np.array_equal(expected, actual):
            raise ValueError(
                f"{self.csv_path}: cache_idx must cover exactly 0..N-1."
            )
    ...
    def __getitem__(self, idx):
        ...
        if not os.path.isabs(str(image_path)):
            image_path = self.data_dir / str(image_path)
...
class RunningLatentStats:
    ...
    def update(self, z: torch.Tensor):
        z = z.detach().to(device="cpu", dtype=torch.float64)

        self.sum += z.sum(dim=(0, 2, 3))
        self.sumsq += (z * z).sum(dim=(0, 2, 3))
        self.count_per_channel += int(z.shape[0] * z.shape[2] * z.shape[3])

    def finalize(self):
        channel_mean = self.sum / self.count_per_channel
        channel_var = self.sumsq / self.count_per_channel - channel_mean**2
        channel_std = torch.sqrt(torch.clamp(channel_var, min=1e-12))

        total_count = self.count_per_channel * self.channels
        global_mean = self.sum.sum() / total_count
        global_var = self.sumsq.sum() / total_count - global_mean**2
...
def get_split_csvs(split_dir: str | os.PathLike):
...
def save_manifest(dataset: EmbedImageDataset, out_dir: Path, split: str):
    # manifest.iloc[i] corresponds to memmap[i]
    manifest = dataset.df.sort_values("cache_idx").reset_index(drop=True)
...
def write_split_latents(
    ...
    if len(dataset) == 0:
        raise RuntimeError(f"Split {split} contains no samples.")
    ...
    out_path = out_dir / f"flux2encoding_float32_{split}.dat"
    ...
    manifest_path = save_manifest(dataset, out_dir, split)
    ...
    for batch in tqdm(loader, desc=f"Encoding {split}"):
        ...
        # get_pretrained_flux2vae() already:
        ...
        if z.ndim != 4:
            raise ValueError(f"Expected z with shape [B,C,H,W], got {z.shape}")
        ...
        if memmap is None:
            ...
            print(f"\n{split} latent shape: {latent_shape}")
            ...
            stats = RunningLatentStats(C)

        z_np = z.cpu().numpy().astype(np.float32, copy=False)

        # cache_idx determines the row in the memmap.
        memmap[cache_idx] = z_np

        # Statistics are calculated on raw latents.
        stats.update(z)

    if memmap is None:
        raise RuntimeError(f"No samples written for {split}.")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Root directory containing the EMBED PNG images",
    )
    parser.add_argument(
        "--out_dir",
        type=str,
        required=True,
        help="Directory to save latent caches",
    )
    ...
    split_csvs = get_split_csvs(args.split_dir)
    ...
    vae = get_pretrained_flux2vae().to(args.device)
    vae.eval()
    ...
    print("Input image size:", (args.img_height, args.img_width))
    ...
    all_meta = {
        "encoder": "black-forest-labs/FLUX.2-dev",
    ...
    # Save registered constants, but do not apply them during encoding.
    if hasattr(vae, "mean"):
        all_meta["vae_registered_mean"] = float(
            vae.mean.detach().float().cpu().item()
        )

    if hasattr(vae, "std"):
        all_meta["vae_registered_std"] = float(
            vae.std.detach().float().cpu().item()
        )
    ...
    for split in ["train", "valid", "test"]:
        ...
        print("latent shape:", meta["latent_shape"])
        print("global mean:", meta["global_mean"])
        print("global std:", meta["global_std"])
    ...
    train_meta = all_meta["splits"]["train"]

    all_meta["train_latent_statistics"] = {
        "global_mean": train_meta["global_mean"],
        "global_std": train_meta["global_std"],
        "per_channel_mean": train_meta["per_channel_mean"],
        "per_channel_std": train_meta["per_channel_std"],
    }
