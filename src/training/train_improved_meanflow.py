...
class Trainer:
    ...
    def train_epoch(self, dataloaders: dict[str, torch.utils.data.DataLoader]) -> float:
        ...
        for _, batch in loader:
            ...
            out = self.model(x, pa, g=None)
            loss, raw_mse = out if isinstance(out, tuple) else (out, None)
            loss.backward()
    ...
    # Use no_grad instead of inference_mode because torch.func.jvp needs
    # forward-mode AD support, which inference tensors can block.
    @torch.no_grad()
    def eval_epoch(
        self,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exp_name", type=str, default="imf_smoke")
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of MeanFlow/iMF sampling intervals used for eval plots.",
    )
